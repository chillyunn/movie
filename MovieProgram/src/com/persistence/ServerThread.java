		...
		while (true) {
			...
			case Protocol.PT_MOVIE:
				...
				case 7: // 영화 삭제 요청
					...
					data = protocol.getData();
					movTitle = data[0];
					protocol = new Protocol(Protocol.PT_MOVIE, 8);
					if (MoviesDAO.EqualTitle(movTitle)) {
						System.out.println("서버DB에 동일한 ID 존재"); // 삭제
				...
				case 9: // 영화관별 상영 시간표 정보 요청
					...
					for (int i = 0; i < result.length; i += 5) {
						...
						protocol = new Protocol(Protocol.PT_MOVIE, 10);
				...
				case 12: // 상영시간표 등록 요청
					...
					movTitle = data[3];
					ttTime = data[4];
					protocol = new Protocol(Protocol.PT_MOVIE, 13);
				...
				case 14: // 상영시간표 삭제 요청
					...
					movTitle = data[3];
					ttTime = data[4];
					protocol = new Protocol(Protocol.PT_MOVIE, 15);
					if (TimeTablesDAO.EqualTime(thtId, scrId, movTitle, ttTime)) {
						System.out.println("서버DB에 동일한 ID 존재"); // 삭제
				...
				break;
			case Protocol.PT_BOOKING:
				String ttid;
				switch (packetCode) {
				case 1:
					System.out.println("클라이언트가 예매좌석 확인 요청을 보냈습니다");
					String[] data = protocol.getData();
					ttid = data[0];
					protocol = new Protocol(Protocol.PT_BOOKING, 2);
					String[] temp = new String[TicketsDAO.selectSeat(ttid).size()];
					for (int i = 0; i < temp.length; i++) {
						temp[i] = TicketsDAO.selectSeat(ttid).get(i);
					}
					protocol.setData(temp);
					os.write(protocol.getPacket());
					System.out.println("예매된좌석 정보전송 완료");
